    ...
    TagsInput.prototype = {
        ...
        add: function (item, dontPushVal, options, surpressItemAdded) {
            ...
            if (typeof item === "string" && this.$element[0].tagName === 'INPUT')
            {
                ...
                if (items.length > 1)
                {
                    // jay: if multiple tags have been entered, then stop at this point and call this function again for each tag
                    // at this point we could instruct the new add() call not to raise the itemAdded event and instead throw our own itemsAdded event
                    // but there's a chance an item might be skipped due to some per-item validation

                    // instead we could pass a call-back that knows to do something when all items processed as it would know how many items there were.
                    var acceptedTags = [];
                    for (var i = 0; i < items.length; i++)
                    {
                        // surpress itemAdded event handler when adding multiple tags
                        if (this.add(items[i], true, null, true)) {
                            // keep track of this tag
                            acceptedTags.push(items[i]);
                        }
                    }
            ...
            else if (!surpressItemAdded)
            {
                // don't do this when multiple tags have been input, we'll emit the itemsAdded event handler instead elsewhere
