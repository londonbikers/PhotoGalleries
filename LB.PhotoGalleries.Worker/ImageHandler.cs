...
namespace LB.PhotoGalleries.Worker
{
    public class ImageHandler
    {
        ...
        private DatabaseId _galleryId;
        private readonly Container _imagesContainer;
        private readonly BlobServiceClient _blobServiceClient;
        private readonly Container _galleriesContainer;
        private static QueueClient _queueClient;
        #endregion

        #region constructors
        public ImageHandler(IConfiguration configuration, ILogger log, BlobServiceClient blobServiceClient, Container galleriesContainer, Container imagesContainer)
        {
            _configuration = configuration;
            _log = log;
            _blobServiceClient = blobServiceClient;
            _galleriesContainer = galleriesContainer;
        ...
        private async Task ProcessImageProcessingMessageAsync(string message)
        {
        ...
        /// </summary>
        private async Task AssignGalleryThumbnailAsync()
        {
            string gid;
            string pk;
        ...
        private async Task<Image> GetImageAsync(DatabaseId databaseId)
        {
        ...
        private async Task<byte[]> GetImageBytesAsync(Image image)
        {
        ...
        private async Task ProcessImageAsync(Image image, byte[] imageBytes, FileSpec fileSpec)
        {
        ...
        private async Task<Stream> GenerateImageAsync(Image image, byte[] originalImage, ImageFileSpec imageFileSpec)
        {
            var stopwatch = new Stopwatch();
            stopwatch.Start();
        ...
        /// </summary>
        private async Task UpdateModelsAsync(Image image)
        {
            var stopwatch = new Stopwatch();
            stopwatch.Start();
